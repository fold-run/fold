...
> ~0.2 ms added p50 on typical hardware; CI fails any merge where the same instrument measures over 5 ms on a shared runner. Under load: ~9,300 req/s of tools/call through one instance at 64 concurrent client sessions (p99 ≤ 19 ms, zero errors) on an M4 Pro over loopback — and the honest caveat is in the table itself: that's 64% of what the same driver gets hitting the upstream directly, because the direct baseline does near-zero work per call. The methodology is in the repo rather than in my comment so you can pick it apart: `make bench` and `make loadtest`, docs/benchmarks.md for the caveats. The rate limits and circuit breakers sit off the happy path — they exist to protect upstreams, not to police clients.
