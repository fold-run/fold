...
Two producers write the discovery document the gateway polls: `fold-discovery` (`internal/kubediscovery`, labeled Kubernetes Services) and `fold-registry` (`internal/mcpregistry`, an allowlist of MCP Registry entries). The gateway cannot tell them apart — the document format is the contract, and both are validated whole against the same allowlists. A third producer belongs beside them rather than inside the gateway.
