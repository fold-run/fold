...
The latest minor release line receives security fixes as patch releases;
older minors are expected to upgrade — upgrades within a major version are
drop-in under the compatibility contract in the README's "API stability"
section. The `main` branch is fixed first; a patch release follows for
