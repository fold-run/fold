...
`"serverInitiatedDecision": "deny"` extends policy to the **reverse direction** — the `sampling/createMessage` and `elicitation/create` requests an upstream makes of the caller's client over a bridged session. Those spend something of the caller's (model budget, or a human's attention), so a rule grants them explicitly: `{ "server": "corpus", "methods": ["sampling/createMessage"] }`, server-and-method only, since neither request has a name. A denial answers the *upstream* with `-32042` — a client is entitled to decline either request — and the audit event carries `direction: "server_initiated"`, which is the one place the exchange is legible to an operator, since the caller never sees it.
