...
POST /mcp
 ...
 → decision hook        optional external allow/deny on ingress (after policy; decides, never rewrites)
 ...
 → egress               per-principal list filtering, namespace rewriting, optional hook on results
...
- **Throughput, measurably** — `make loadtest` sweeps one instance under 8/64/256 concurrent SDK client sessions, direct vs through-fold: **~8,500 req/s `tools/call` at 64 connections (p99 ≤ 22 ms), 12,100 req/s at 256 (p99 ≤ 65 ms), zero errors**, with the gateway process at **≈26 MiB idle and ≈0.3–0.4 MiB plus ≈7 goroutines per held session** (Apple M4 Pro, loopback, `main` 2026-09-04). Methodology, full tables, memory, and the honest caveats: [docs/benchmarks.md](docs/benchmarks.md).
...
- [docs/design-policy-depth.md](docs/design-policy-depth.md) — design record (shipped): deny rules, argument constraints, and destructive-operation gating — with the precedence, visibility, and trust questions each one raises settled before implementation, and the two things implementation decided that the design did not anticipate.
- [docs/design-decision-hook.md](docs/design-decision-hook.md) — design record (shipped): the external decision hook — why it decides and cannot rewrite, why `timeoutMs` and `onError` have no defaults, and why sending arguments to a second endpoint is a data-egress decision to state in those words.
...
- **A stdio front door** — fold's client-facing transport is streamable HTTP only. `fold-stdio` puts a *stdio server* behind the gateway; nothing puts the gateway in front of a *stdio-only client*, which needs a bridge such as `mcp-remote`. This is a decision rather than a gap awaiting work — a local process per user sits awkwardly with "one governed endpoint" and complicates principal attribution and audit — and it is recorded as a non-goal in [docs/roadmap.md](docs/roadmap.md#non-goals) and in [docs/design-stdio.md](docs/design-stdio.md).
