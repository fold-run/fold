...
**Conformant, provably.** The official [`@modelcontextprotocol/conformance`](https://github.com/modelcontextprotocol/conformance) suite runs against fold fronting the reference everything-server on every merge — **40/40 checks**, including sampling, elicitation, logging, progress, and resource subscriptions bridged through the gateway. The receipt is one click away: the [`conformance` job from the v1.13.0 release run](https://github.com/fold-run/fold/actions/runs/31833840044/job/94875553255), and [every green run on `main`](https://github.com/fold-run/fold/actions/workflows/ci.yml?query=branch%3Amain+is%3Asuccess). A [weekly job](.github/workflows/drift.yml) re-runs the suite against the *latest* unpinned SDK and opens a tracking issue the moment anything drifts. Reproduce it yourself with `./scripts/conformance.sh` — the same command CI runs.
...
- **MCP Apps capability negotiation** — the routing half of [MCP Apps](https://modelcontextprotocol.io/extensions/apps/overview) ships (above); the handshake half does not. The extension is *client-declared* — a server is advised to check `capabilities.extensions["io.modelcontextprotocol/ui"]` before registering UI-enabled tools — and fold's upstream sessions declare nothing, so an upstream that takes that advice serves its text-only fallback to a host that asked fold for apps. Measured, not inferred, against a real gated upstream; no server in the ext-apps examples gates yet, which is why apps work through fold at all today. The fix is to proxy the declaration rather than configure it, and the shared root session is what makes it design work rather than wiring: [docs/design-mcp-apps.md](docs/design-mcp-apps.md) settles it on a capability-keyed session, with no config field. Two further gaps are the protocol's rather than fold's: an app that hardcodes a tool name cannot call it once federation has namespaced it (verified — the reference host forwards the bare name and fold answers `-32043`), and the host's rule that an app may not call across servers has no meaning when every upstream arrives on one connection, since the spec defines no way for a gateway to tell an app-initiated call from a model-initiated one. Those two are reports to file, not features to build.
