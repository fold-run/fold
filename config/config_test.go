...
func TestValidationErrors(t *testing.T) {
	cases := []struct {
		...
		{"console oauth without auth", `{"upstreams":[{"id":"a","url":"http://x.test"}],"server":{"console":{"enabled":true,"oauth":{"clientId":"c"}}}}`, `auth.mode "required"`},
		{"console oauth without clientId", `{"upstreams":[{"id":"a","url":"http://x.test"}],"server":{"console":{"enabled":true,"oauth":{"clientId":""}}},"auth":{"mode":"required","resource":"https://gw.test","issuers":[{"issuer":"https://idp.test"}]}}`, "clientId"},
		{"console oauth unknown issuer", `{"upstreams":[{"id":"a","url":"http://x.test"}],"server":{"console":{"enabled":true,"oauth":{"clientId":"c","issuer":"https://other.test"}}},"auth":{"mode":"required","resource":"https://gw.test","issuers":[{"issuer":"https://idp.test"}]}}`, "direct-mode auth issuer"},
		{"console oauth only exchange issuers", `{"upstreams":[{"id":"a","url":"http://x.test"}],"server":{"console":{"enabled":true,"oauth":{"clientId":"c"}}},"auth":{"mode":"required","resource":"https://gw.test","issuers":[{"issuer":"https://idp.test","mode":"exchange"}],"ema":{"idpIssuer":"https://idp.test","signingKeyRef":"K"}}}`, "direct-mode issuer"},
