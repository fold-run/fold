...
**Load test** (`make loadtest`): three processes, so the load generator, the gateway, and the upstream never share a scheduler. The gateway is the real production entry — the built `./cmd/fold` binary with a config file. The driver models real MCP clients: each "connection" is an official-SDK client session (initialize once, then sequential `tools/call`), because fold's client side is session-keyed — a load test should measure what a real client experiences, including the SDK on both ends. Every stage runs DIRECT (driver → upstream) and FOLD (driver → gateway → upstream), so the upstream's own ceiling is visible: fold's RPS can never exceed direct's, and the gap is the honest cost. 3 s warmup, 10 s measured, connection sweep 8 / 64 / 256, client retries disabled (one call, one request). At the end of every fold stage the driver scrapes the gateway's own `/metrics` for resident memory, goroutines, and live downstream sessions, and when the run ends it reads the gateway process's peak RSS from the kernel (`ru_maxrss`) — the numbers a memory limit is set from, and the reason the JSON now carries a `gatewayIdle` baseline and a `gatewayPeakRssBytes` total.
...
## The numbers (`main` @ `df69d9e`, 2026-09-04)

*Re-measured after v1.15.0 and the production-readiness work that followed it; the previous tables were the v1.3.0 run of 2026-08-05.*
...
|---|---|---|---|---|---|
| 8 | 11,810 | **7,081** | 1.1 ms | 2.4 ms | 60% |
| 64 | 13,218 | **8,505** | 6.7 ms | 21.3 ms | 64% |
| 256 | 21,271 | **12,066** | 18.9 ms | 64.6 ms | 57% |

Zero errors across the sweep. Passthrough mode (single un-namespaced upstream) measures within noise of namespaced: 6,130 / 7,596 / 11,682 req/s at the same three points.

Headline, stated the way we'd defend it on a stage: **one fold instance sustained ~8,500 req/s of `tools/call` at 64 connections with p99 ≤ 22 ms — 12,100 req/s at 256 connections with p99 ≤ 65 ms — zero errors across the sweep, at 57–64% of the upstream's own direct ceiling.** The v1.3.0 run was ~9,300 / 13,400; the difference is twelve releases of governance now done on every call — tenant resolution, the visibility subset, consumption metering, definition pinning, the body read deadline — and is within the run-to-run spread of a shared laptop.

### Memory and goroutines (the gateway process, namespaced sweep)

What the process weighs, which is what a memory limit is set from. Scraped from the gateway's own `/metrics` at the end of each fold stage; `sessions` is `fold_downstream_sessions`.

| stage | live conns | sessions held | RSS | goroutines |
|---|---|---|---|---|
| idle, before the sweep | 0 | 0 | **25.8 MiB** | 22 |
| `tools/call` | 8 | 10 | 36.5 MiB | 83 |
| `tools/call` | 64 | 74 | 59.4 MiB | 555 |
| `tools/call` | 256 | 330 | 152.6 MiB | 2,463 |
| `tools/list` | 8 | 338 | 120.0 MiB | 2,475 |
| `tools/list` | 64 | 402 | 161.6 MiB | 2,595 |
| `tools/list` | 256 | 658 | 170.2 MiB | 3,046 |
| **peak over the run** (`ru_maxrss`) | | | **174.6 MiB** | |

Read the `sessions held` column before the RSS one. Each stage's clients close their sessions when it ends, and in this run the count still climbed — which turned out to be a defect, not a property: a default SDK client probes the modern era with a session-less `server/discover` before it initializes, the stateful handler minted a session for the probe, and the client's fallback `initialize` opened a second one and only ever deleted that. Every connecting client left one orphan. fold now refuses the probe the way its own upstream leg always has (a 405 the SDK treats as "fall back"), so the column tracks live clients; the table is kept as measured because it is the run that found the bug, and its per-session costs are unchanged by the fix. What remains true is the case the column also shows: **a downstream session whose client disconnects without a `DELETE` stays on the gateway — with its bridged upstream session and their goroutines — until `server.sessionIdleTimeoutMs` (30 minutes by default) expires it.** So a gateway's memory is set by its *peak* population of sessions over the last half hour, not by its current connection count, and a load spike from clients that drop rather than close is paid for until the spike's sessions age out. The per-session cost is steady across the table: **≈0.3–0.4 MiB of RSS and ≈7 goroutines per held session** (a live one costs the higher figure, an abandoned one the lower), on top of a ≈26 MiB idle floor. That arithmetic is what the [Sizing](operations.md#sizing) section is built on, and it is why the chart's `256Mi` default limit is documented as good for roughly 600 held sessions rather than as a measurement of anything smaller.

### Federation cost (`tools/list`, gateway-side, warm caches)

*Measured 2026-08-15 on `main`, after v1.14.0 and including the allocation fix noted below; the `tools/call` and memory tables above are from the 2026-09-04 run.*
...
- **Sessions outlive clients that drop.** A client that closes its session (`DELETE`) frees it at once; one that disconnects without closing leaves it to the idle timeout. Size memory to the peak session population over `sessionIdleTimeoutMs`, and shorten that timeout for a client population that drops rather than closes. (The measured table's climb was the orphan-probe defect, since fixed; the fix does not change the per-session cost.)
