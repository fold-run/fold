...
Read the `sessions held` column before the RSS one. Each stage's clients close their sessions when it ends, and the count still climbs: **a downstream session the client has abandoned stays on the gateway — with its bridged upstream session and their goroutines — until `server.sessionIdleTimeoutMs` (30 minutes by default) expires it.** So a gateway's memory is set by its *peak* population of sessions over the last half hour, not by its current connection count, and a load spike is paid for until the spike's sessions age out. The per-session cost is steady across the table: **≈0.3–0.4 MiB of RSS and ≈7 goroutines per held session** (a live one costs the higher figure, an abandoned one the lower), on top of a ≈26 MiB idle floor. That arithmetic is what the [Sizing](operations.md#sizing) section is built on, and it is why the chart's `256Mi` default limit is documented as good for roughly 600 held sessions rather than as a measurement of anything smaller.
...
- **Sessions outlive their clients.** The memory table shows sessions accumulating across stages whose clients have all disconnected: an abandoned session is reclaimed by the idle timeout, not by the disconnect. Size memory to the peak session population over `sessionIdleTimeoutMs`, and shorten that timeout for a client population that churns.
