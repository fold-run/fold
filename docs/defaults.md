...
| `server.allowedOrigins` | absent (derived from `allowedHosts`) | The derived rule is right for every deployment that names its own hostname, and the field exists for the one that does not: `allowedHosts: ["*"]` behind a proxy, where the derivation disappears and MCP's Origin requirement with it. Defaulting to same-origin instead would have refused the browser clients — Inspector on `localhost:6274` — that a wildcard deployment serves today, which is a changed meaning. So the default stays, the gap is warned about at startup, and the additive field closes it. |
...
| `discovery.allowedUpstreamHosts` | absent (unrestricted) | Same reasoning as the row above, for the bound the credential allowlists leave open: an uncredentialed upstream at any host. A default would have to name hosts fold cannot guess. Absent is warned about at startup; the deploy checklist lists it beside the other three. |
