...
| `GET /health` | always | Pings every upstream concurrently (5 s internal budget); `503` when none of the *probeable* ones is reachable. An upstream whose credential is derived from the caller (`passthrough`, `token-exchange`) has none outside a request, so it is reported `"unprobed": true` rather than down and counts as neither healthy nor unhealthy — a federation made entirely of them answers `200`, because there is nothing the gateway can check and nothing it has found wrong. The one *empty* federation that answers `503` is a discovery-driven one whose source has never been applied: with no static upstreams and `discovery` configured, the response carries a `discovery` object (`lastOutcome`, `lastSyncAt`, `applied`) and stays `503` until a document is applied — a source that was down when the pod started must not put a pod that serves empty lists into rotation. A source that applies an *empty* list is `200`: it said so. `/healthz` — the path through v1.4, kept as a deprecated alias through v1.8 — was **removed in v1.9**; point probes at `/health`. The fan-out is single-flighted and reused for one second, so a poll loop against this unauthenticated endpoint costs at most one collection per second (a reload or discovery sync invalidates immediately). URLs, owners, labels, and raw error text are always withheld — the endpoint is unauthenticated, so no caller is trusted with them; the detailed view is `/api/federation`'s, behind authentication. Multi-endpoint upstreams include a per-replica `endpoints` array with the balancer's rotation state. |
...
**Startup warnings worth grepping for.** Each names a posture fold accepts
and an operator should have chosen on purpose: `auth is disabled and the
listener is bound beyond loopback` (an open gateway); `credentialed upstream
