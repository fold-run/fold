...
| `GET /health` | always | Pings every upstream concurrently (5 s internal budget); `503` when none is reachable. The fan-out is single-flighted and reused for one second, so a poll loop against this unauthenticated endpoint costs at most one collection per second (a reload or discovery sync invalidates immediately). Detailed fields (URLs, owners, labels, error text) appear only when auth is disabled, so an unauthenticated caller on a public deployment cannot enumerate the federation. Multi-endpoint upstreams include a per-replica `endpoints` array with the balancer's rotation state. |
