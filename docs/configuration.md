...
| `allowedHosts` | localhost set | DNS-rebinding protection: allowed Host/Origin hostnames. Set to your public hostname(s) in production, or `["*"]` only behind a trusted proxy — and then set `allowedOrigins`, because the wildcard switches the derived Origin rule off with it (fold warns at startup when it has). |
| `allowedOrigins` | derived from `allowedHosts` | The Origin allowlist for browser-originated requests, independent of Host handling. Absent, an Origin must name an allowed host, which is right for a gateway serving its own hostname and vanishes under `allowedHosts: ["*"]`. Set, it *replaces* that derivation: entries are exact hostnames or `"*.suffix"` matched against the Origin's host (ports ignored), and `["*"]` accepts any Origin — the explicit form of what the Host wildcard used to imply. Requests with no Origin (non-browser clients) are unaffected either way. Construction-wired. |
...
```jsonc
{
  ...
  "allowedUpstreamHosts": ["*.svc.cluster.local"],         // optional: where ANY discovered upstream may live, credentialed or not (absent → unrestricted)
...
`allowedUpstreamHosts` is the fourth bound, and it covers what the three above do not: **an upstream with no credential at all**. Registering one costs a source no secret, and it still puts that host's tool names and descriptions in front of every model behind the gateway and has the gateway dial it on every list fan-out. The credential allowlists bound where a *secret* travels; this bounds where *traffic* travels. Same pattern rules; same fail-safe rejection; absent means unrestricted, and fold warns at startup when discovery is configured without it. All four are construction-wired — a change needs a restart, and `Reload` refuses one.
